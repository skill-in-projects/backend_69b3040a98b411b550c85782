...
get '/api/google/speech-to-text' do
  ...
  begin
    ...
    http = Net::HTTP.new(uri.host, uri.port); http.use_ssl = true; http.open_timeout = 10; http.read_timeout = 10
    req = Net::HTTP::Post.new(uri); req['Content-Type'] = 'application/json'; req.body = "{"config":{"encoding":"LINEAR16","sampleRateHertz":16000,"languageCode":"en-US"},"audio":{"content":"#{base64_audio}"}}"
    res = http.request(req)
    body = res.body
